...
package providers.playkit.kaltura.com.mediaproviders.api.base.model;
...
public class BasePlaybackContext extends BaseResult {
    ...
    public ErrorElement hasError() {
        ...
        if (hasBlockedAction() && messages != null) {
            ...
            for (KalturaAccessControlMessage message : messages) {
    ...
    public boolean hasBlockedAction() {
        if (actions != null) {
            for (KalturaRuleAction rule : actions) {
                if (rule != null && KalturaRuleAction.KalturaRuleActionType.BLOCK.equals(rule.getType())) {
                    return true;
                }
            }
        }
